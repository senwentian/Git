...
uint32_t Gyro_Message;
uint16_t Gyro_Temperature;
uint32_t Angular_velocity_z;
...
void Gyro_Info(void)
{
    if((Gyro_Msg[4] & 0x08) == 0x08) {
        //negative
        Gyro_Message = (uint32_t)(~((Gyro_Msg[0]  |  Gyro_Msg[1] << 7  |  Gyro_Msg[2] << 14  |  Gyro_Msg[3] << 21  |  Gyro_Msg[4] << 28) & 0xFFFF)) + 1;
    } else {
        //positive
    ...
    printf("the angular velocity of 'z' is %d\nthe Gyro Temperature is %d\n", Angular_velocity_z, Gyro_Temperature);
}
